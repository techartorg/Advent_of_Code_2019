...
understands parameter mode 0, position mode, which causes the parameter to be interpreted as a
...
on the ones and tens digit of the value, that is, the opcode is the rightmost two digits of the first value
in an instruction. Parameter modes are single digits, one per parameter, read right-to-left from the opcode:
...
works like it did before - its value is the value stored at address 4 (33). The second parameter, 3 in immediate mode,
...
result of the test was from the expected value, where 0 means the test was successful.
...
def run_intcode(input_data, input_code=None):
    ...
    while program[i] != 99:
        ...
        if instruction == 1:
            ...
            program[program[i + 1]] = int(input("INPUT: ")) if input_code is None else input_code
        elif instruction == 4:
            print(operands[0])
        elif instruction == 5:
