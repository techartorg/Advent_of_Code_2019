...
the image left-to-right, then move downward to the next row, filling rows top-to-bottom until every
...
if __name__ == "__main__":
    data = open("puzzle_inputs/input_day_08.txt", 'r').read()

    width, height = 25, 6
    layer_length = width * height

    layers = [data[i * layer_length:(i + 1) * layer_length] for i in range(len(data) // layer_length)]
    ...
    img = np.zeros((height, width), dtype=np.uint8) + 2
    for i in range(len(data) // layer_length):
        layer = list(data[i * layer_length: (i + 1) * layer_length])
        np_layer = np.array([int(x) for x in layer], dtype=np.uint8).reshape((height, width))
        np.copyto(img, np_layer, where=(img == 2))

    print("Part Two:", str(img).replace('1', u"\u2588").replace('0', ' '))
