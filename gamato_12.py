...
position coordinates. A moon's kinetic energy is the sum of the absolute values of its velocity coordinates.
...
In the above example, adding together the total energy for all moons after 10 steps produces the total energy
in the system, 179.
...
def lcm(x, y):
    """
    Calculate the least common multiply of many
    :param int x: value
    :param int y: value
...
def get_velocity(vectors):
    velocity = []
    for vector in vectors:
        v = sum(vectors > vector) - sum(vectors < vector)
        velocity.append(v)

    return np.array(velocity)


def calc_total_energy(vectors, steps):
    velocity = np.zeros((4, 3)).astype(int)
    for i in range(steps):
        velocity += np.apply_along_axis(get_velocity, axis=0, arr=vectors)
        vectors += velocity
        if i == steps - 1:
            return np.sum(np.sum(np.abs(vectors), axis=1) * np.sum(np.abs(velocity), axis=1))


def calc_num_efficiency_steps(vectors, max_steps=10000000):
    initial_vectors = vectors.copy()

    velocity = np.zeros((4, 3)).astype(int)
    num_cycles = [0, 0, 0]
    has_cycled = [False, False, False]

    for i in range(max_steps):
        velocity += np.apply_along_axis(get_velocity, axis=0, arr=vectors)
        vectors += velocity
        for j in range(vectors.shape[1]):
            if (vectors[:, j] == initial_vectors[:, j]).all() and not has_cycled[j]:
                num_cycles[j] = i + 2
                has_cycled[j] = True
        if all(has_cycled):
            return reduce(lcm, num_cycles, 1)


if __name__ == "__main__":
    ex = re.compile(r'<x=(.*), y=(.*), z=(.*)>')

    data = open('input12.txt', 'r').read().strip().split('\n')
    moon_vectors = np.asarray([list(map(int, ex.findall(data[i])[0])) for i in range(len(data))])
