...
the first integer (called position 0). Here, you will find an opcode - either 1, 2, or 99.
...
until you add more instructions to the computer, this is always 4 (1 opcode + 3 parameters) for the add and multiply
...
just like before. In this program, the value placed in address 1 is called the noun, and the value placed
...
Each time you try a pair of inputs, make sure you first reset the computer's memory to the values in
...
def specific_code_program(memory: List[int], code: int) -> int:
    for i, j in itertools.product(range(100), range(100)):
        if intcode_program(memory[:], i, j) == code:
            return 100*i+j


def intcode_program(memory: List[int], noun: int, verb: int) -> int:
    memory[1] = noun
    memory[2] = verb
    addr = 0

    while memory[addr] != 99:
        operator, idx, jdx, store = memory[addr:addr+4]
        if operator == 1:
            memory[store] = memory[idx] + memory[jdx]
        elif operator == 2:
            memory[store] = memory[idx] * memory[jdx]
        addr += 4
    return memory[0]
...
if __name__ == "__main__":
    data = [int(value) for value in open('puzzle_inputs/input_day_02.txt', 'r').read().split(',')]
    print("Part One:", intcode_program(data[:], 12, 2), "Part Two:", specific_code_program(data[:], 19690720))
